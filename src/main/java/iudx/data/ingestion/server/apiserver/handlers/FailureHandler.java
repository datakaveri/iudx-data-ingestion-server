...
public class FailureHandler implements Handler<RoutingContext> {
  ...
  public void handle(RoutingContext context) {
    ...
    if (failure instanceof DxRuntimeException) {
      ...
      JsonObject response = new RestResponse.Builder().withType(exception.getUrn().getUrn())
          .withTitle(code.getDescription()).withMessage(code.getDescription()).build().toJson();
  ...
  private JsonObject validationFailureResponse(String message) {
    ...
    return new JsonObject().put(Constants.JSON_TYPE, badReq.getUrn())
        .put(Constants.JSON_TITLE, badReq.getMessage()).put(Constants.JSON_DETAIL, message);
  }
