...
public class RabbitClient {
  ...
  public Future<JsonArray> publishMessage(JsonArray request, JsonObject metaData) {
    ...
    LOGGER.info(
        "Sending each message separately to exchange: {}, with routing key: {}",
        exchangeName,
        routingKey);
  ...
  public Future<Boolean> populateExchangeCache(
      String virtualHost, Cache<String, Boolean> exchangeListCache) {
  ...
  public Future<JsonObject> getExchange(
      String exchange, String virtualHost, Boolean doesExchangeExist) {
              ...
              if (result.succeeded()) {
                ...
                if (statusCode == HttpStatus.SC_CREATED) {
                  ...
                  if (exchangeName == null || routingKey == null) {
                    LOGGER.error(
                        "Exchange name or routing key is null. Cannot proceed with binding.");
                    promise.fail("Exchange name or routing key is null.");
                    return;
                  }

                  JsonObject bindingRequest =
                      new JsonObject()
                      ...
                      .onComplete(
                          bindResult -> {
                            if (bindResult.succeeded()) {
                              ...
                              LOGGER.warn(
                                  "Binding request failed for Default Queue: {}",
                                  bindResult.cause());
                            }
                          });
  ...
  public Future<JsonObject> bindQueue(JsonObject request, String virtualHost) {
    ...
    String routingKey = request.getString(ROUTING_KEY);
    String url = "/api/bindings/" + virtualHost + "/e/" + exchangeName + "/q/" + queueName;
  ...
  public Future<JsonObject> setTopicPermissions(
      JsonObject request, String virtualHost, String userId) {
