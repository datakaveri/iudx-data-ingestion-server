...
/*
    ...
    lenient().when(rabbitWebClient.requestAsync(anyString(),anyString())).thenReturn(httpResponseFuture);
    lenient().when(httpResponseAsyncResult.succeeded()).thenReturn(true);
    when(bufferHttpResponse.bodyAsJsonObject()).thenReturn(new JsonObject());
*/
    ...
    public void createExchangeTest400(VertxTestContext vertxTestContext){
        ...
        rabbitClient
                ...
                .onComplete(
                        handler -> {
                            if (handler.succeeded()) {
                                assertEquals(EXCHANGE_EXISTS_WITH_DIFFERENT_PROPERTIES, handler.result().getString("detail"));
                                assertEquals("failure", handler.result().getString("title"));
                                vertxTestContext.completeNow();
                            } else {
                                handler.failed();
        ...
        rabbitClient
                ...
                .onComplete(
                        handler -> {
                            if (handler.succeeded()) {
                                assertEquals(FAILURE, handler.result().getString("title"));
                                assertEquals(QUEUE_ALREADY_EXISTS_WITH_DIFFERENT_PROPERTIES, handler.result().getString("detail"));
                                vertxTestContext.completeNow();
                            } else {
                                handler.failed();
                            }
    ...
    public void populateExchangeCacheTest(VertxTestContext vertxTestContext){
        ...
        doAnswer(new Answer <HttpResponse<Buffer>>() {
            @Override
        ...
        }).when(httpResponseFuture).onSuccess(any());
       ...
       *//* when(httpResponseAsyncResult.succeeded()).thenReturn(false);
        ...
        }).when(httpResponseFuture).onFailure(any());*//*

        rabbitClient.populateExchangeCache(vHost,exchangeListCache).onComplete(
                handler->{
                    if (handler.succeeded()){
                        vertxTestContext.completeNow();
                    }
